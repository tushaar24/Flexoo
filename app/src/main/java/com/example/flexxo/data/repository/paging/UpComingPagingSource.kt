...
class UpComingPagingSource(private val moviesService: MoviesService) : PagingSource<Int, Movie>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Movie> {
        return try {
            val page = params.key ?: 1
            val response = moviesService.getUpComingMovies(Constants.API_KEY, page).results
            LoadResult.Page(
                response,
